...
!define bolFirefox116OrLater true ;Comment out when not needed
...
Var strTaskBarHash
Var strCityHash
!ifdef bolFirefox116OrLater
	Var bolDisableOSCheck
!endif
...
!ifdef bolFirefox116OrLater
	${If} ${FileExists} "$EXEDIR\${NAME}.ini"
		${ReadINIStrWithDefault} $bolDisableOSCheck "$EXEDIR\${NAME}.ini" "${NAME}" "DisableOSCheck" "false"
	${EndIf}
	
	${If} $bolDisableOSCheck != "true"
		ReadRegStr $0 HKLM "Software\Microsoft\Windows NT\CurrentVersion" "CurrentBuild"
		${If} $0 < 10000
			${GetParent} $EXEDIR $1
			${If} ${FileExists} "$1\FirefoxPortableLegacy115\FirefoxPortable.exe"
				MessageBox MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON1 `Firefox 116 and later requires Windows 10 or later. Would you like to run Firefox Legacy 115 instead?` IDYES CustomCompatibilityCheckRunLegacy IDNO CustomCompatibilityCheckGotoAbort
			${Else}
				MessageBox MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON1 `Firefox 116 and later requires Windows 10 or later. Would you like to download Firefox Legacy 115?` IDYES CustomCompatibilityCheckGotoURL IDNO CustomCompatibilityCheckGotoAbort
			${EndIf}
			CustomCompatibilityCheckRunLegacy:
				Exec "$1\FirefoxPortableLegacy115\FirefoxPortable.exe"
				Abort
			
			CustomCompatibilityCheckGotoURL:		
				ExecShell "open" "https://portableapps.com/apps/internet/firefox-portable-legacy-115"
				Abort	
			
			CustomCompatibilityCheckGotoAbort:
				Abort
		${EndIf}
	${EndIf}
!endif
...
Section "Main"
	...
	FixPrefsJsPart2:
		...
		FileWriteByte $0 "13"
		FileWriteByte $0 "10"
		FileWrite $0 `user_pref("browser.privacySegmentation.createdShortcut", true);`
		FileWriteByte $0 "13"
		FileWriteByte $0 "10"
	...
	;CheckMultipleInstances:
		...
		${If} ${ProcessExists} "firefox.exe"
			;${IfNot} ${FileExists} "$PROFILEDIRECTORY\.startup-incomplete"
			;	${If} ${FileExists} "$PROFILEDIRECTORY\sessionstore.jsonlz4"
	...
	RemoveLocalFiles3:
		...
		Delete "$PROGRAMDATA\Mozilla-1de4eec8-1241-4177-a864-e594e8d1fb38\cache2.*-$strTaskBarHash"
